...
docker compose --project-directory "$SCRIPT_DIR" --profile prod up -d --build

printf '%s\n' ''
printf '%s\n' 'Deployment started.'
printf '%s\n' 'Check status with:'
printf '  %s\n' "docker compose --project-directory \"$SCRIPT_DIR\" --profile prod ps"
printf '%s\n' 'Check logs with:'
printf '  %s\n' "docker compose --project-directory \"$SCRIPT_DIR\" --profile prod logs -f backend-prod"
printf '  %s\n' "docker compose --project-directory \"$SCRIPT_DIR\" --profile prod logs -f nginx"
